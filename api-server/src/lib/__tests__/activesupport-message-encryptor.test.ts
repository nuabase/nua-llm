...
const ENCRYPTED_PAYLOAD =
  "0UspvJr1z2r5C7+xmku9SyKuom2fruXXKyLJ5+WPt5deo6SAgAv4mZjHqzw7--LMQ/fKOY3R3mZik9--hcsuBb6inK5SZCIVJApPCw==";
...
beforeAll(() => {
  ...
  ({
    decryptRailsMessageEncryptor,
  } = require("../activesupport-message-encryptor"));
});

describe("decryptRailsMessage", () => {
  it("returns the parsed object with a valid payload and key", () => {
    const decrypted = decryptRailsMessageEncryptor(
      ENCRYPTED_PAYLOAD,
      ENCRYPTION_KEY,
    );
